FROM ubuntu:18.04
RUN apt-get update &&\
apt-get install -y openjdk-11-jdk curl wget git wget curl openssl net-tools locales maven dumb-init sudo build-essential gradle &&\
...
RUN tar -xf *.tar.gz && mv code-server*/code-server /usr/local/bin/
